...
/*   Updated: 2025/02/03 14:02:50 by rduro-pe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_decor(t_game *game)
{
	new_sprite_to_bg(game, game->sprite->speaker, (t_cord){45, 45},
		(t_cord){30 + game->gap, game->map->heigth
		* game->size - game->gap});
	new_sprite_to_bg_hflip(game, game->sprite->speaker, (t_cord){45, 45},
		(t_cord){(game->map->width + 1) * game->size + game->gap + 15 + game->offset,
		game->map->heigth * game->size - game->gap});
	if (game->map->heigth <= 10 && game->map->width <= 19)
		return ;
	if (game->map->width <= 34)
		new_sprite_to_bg(game, game->sprite->logo, (t_cord){180, 180},
			(t_cord){37 * game->size, 1 * game->size});
	if (game->map->width <= 34 || game->map->heigth <= 16)
		new_sprite_to_bg(game, game->sprite->button, (t_cord){180, 180},
			(t_cord){37 * game->size, 18 * game->size});
	if (game->map->heigth <= 16)
		new_sprite_to_bg(game, game->sprite->arrow, (t_cord){180, 180},
...
void	put_sprite(t_game *game, int y, int x, int type)
{
	if (type == 'F')
		colorblock_to_bg(game, 0xC7C7C7, (t_cord){game->size, game->size},
			(t_cord){((x + 1) * game->size) + game->gap + game->offset, (y + 1) * game->size});
	else if (type == 'O' || type == 'c' || type == 'E' || type == 'P')
		new_sprite_to_bg(game, game->sprite->empty_1, (t_cord){game->size,
			game->size}, (t_cord){((x + 1) * game->size) + game->gap + game->offset, (y + 1)
			* game->size});
	else if (type == '1')
